...
#[get("/?<client_address>")]
pub fn get_clientlocation(mut connection: db::JaspyDB, client_address: String) -> Option<Json<models::dbo::ClientLocation>> {
    if let Some(existing_client_info) = models::dbo::ClientLocation::by_ip(&client_address, &mut connection) {
        return Some(Json(existing_client_info));
    } else {
        return None;
    }
}
...
pub fn put_clientlocation(mut connection: db::JaspyDB, client_location_info_json: Json<models::json::ClientLocationInfo>) {
    ...
    if option82_001_hex.len() == 6 && option82_002_hex.len() == 8 {
        ...
        let device;
        if let Some(some_device) = models::dbo::Device::by_base_mac(&switch_base_mac, &mut connection) {
            device = some_device;
        } else {
            return;
        }
        if let Some(mut existing_client_info) = models::dbo::ClientLocation::by_ip(&client_location_info.yiaddr, &mut connection) {
            if existing_client_info.port_info != port_info || existing_client_info.device_id != device.id {
                ...
                if let Err(_update_error) = existing_client_info.update(&mut connection) {
                    // TODO: log
                }
            }
        } else {
            ...
            if let Ok(_new_client_location) = models::dbo::ClientLocation::create(&l, &mut connection) {
            } else {
                // TODO: log this
            }
        }
